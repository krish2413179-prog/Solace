...
import {
  getProvider, getWallet, getSolace, getRegistry,
  sendTx, generatePipelineId, sleep,
} from "./utils/chain.js";
...
function getLocalSwarm(): Set<string> {
  try {
...
const STEP_STATUS_NAMES: Record<number, string> = {
...
async function discoverForStep(
  ...
  if (step.capability) {
    try {
      candidates = (await registry.getAgentsByCapability(step.capability) as string[])
  ...
  if (!candidates.length) {
    candidates = (await registry.getAvailableAgents() as string[])
      .filter((a: string) => !excluded.has(a.toLowerCase()));
  }
  ...
  for (const addr of candidates) {
    const isAvail = await registry.isAvailable(addr) as boolean;
    if (!isAvail) continue;
    const score = Number(await registry.getScore(addr));
    if (score >= minScore) scored.push({ addr, score });
  }

  if (!scored.length) throw new Error(`No eligible agent for step ${step.id} (cap: ${step.capability ?? "any"})`);
...
async function collectCommits(
  ...
  for (const msg of msgs) {
    ...
    commits.set(wallet, msg.payload.commit_hash as string);
    logger.info(`  ${wallet.slice(0, 12)}... → ${(msg.payload.commit_hash as string).slice(0, 16)}...`);
  }
...
async function createPipeline(
  ...
  const orderedHashes = assignments.map(a =>
    bytes32FromHex(commits.get(a.wallet.toLowerCase())!)
  );

  logger.info(`Locking ${orderedHashes.length} commitment(s)...`);
  await sendTx(
    () => solace.lockCommitments(pipelineId, orderedHashes),
    "lockCommitments",
  );
}
...
async function monitorPipeline(
  ...
  while (true) {
    const [, , , , delivered, accepted, total, , status] =
    ...
    if (pStatus === PIPE_STATUS.Settled)    { logger.info("SETTLED — all agents paid"); return "settled"; }
    if (pStatus === PIPE_STATUS.RolledBack) { logger.warn("ROLLED BACK — bounty refunded"); return "rolled_back"; }

    for (let i = 0; i < stepCount; i++) {
      try {
        const step    = await solace.getStep(pipelineId, i);
        const sStatus = Number(step[4]);
        logger.info(`    Step ${i}: ${STEP_STATUS_NAMES[sStatus] ?? sStatus} | agent: ${(step[0] as string).slice(0, 12)}...`);
      } catch {
...
async function main() {
  ...
  if (!taskFile) throw new Error("Usage: npm run orchestrator <task_file.json>");
