...
function prune() {
  ...
  for (const [key, msgs] of messageStore.entries()) {
    messageStore.set(
      key,
      msgs.filter((m) => m.timestamp * 1000 > cutoff)
    );
  }
}
...
async function startNode() {
  ...
  node.services.pubsub.addEventListener("message", (evt) => {
    ...
    try {
      ...
      if (!messageStore.has(key)) messageStore.set(key, []);
      messageStore.get(key)!.push(msg);

      console.log(
        `[${new Date().toISOString().slice(11, 19)}] RECEIVED ${msg.msg_type} from ${msg.sender.slice(0, 10)}...`
      );
    } catch (err) {
...
async function publishMessage(msg: Message): Promise<void> {
  const msgStr = JSON.stringify(msg);
  const msgBytes = uint8ArrayFromString(msgStr);
  await node.services.pubsub.publish(TOPIC, msgBytes);
  console.log(
    `[${new Date().toISOString().slice(11, 19)}] PUBLISHED ${msg.msg_type} (${msg.sender.slice(0, 10)}...)`
  );
}

function getMessages(msgType?: string, after = 0): Message[] {
  prune();
...
const httpServer = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/health") {
    ...
    const msgCount = Array.from(messageStore.values()).reduce(
      (a, b) => a + b.length,
      0
    );
