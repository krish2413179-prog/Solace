...
const brokerServer = http.createServer(async (req, res) => {
    ...
    const prune = (id) => {
        ...
        channels.set(id, (channels.get(id) ?? []).filter((m) => m.timestamp * 1000 > cutoff));
    };
    if (url === '/health')
        return send(200, { status: 'ok', transport: 'http-rest' });
    ...
    if (action === 'messages' && method === 'GET') {
        ...
        let msgs = (channels.get(channelId) ?? []).filter((m) => m.timestamp > after);
        if (msgType)
            msgs = msgs.filter((m) => m.msg_type?.toUpperCase() === msgType);
...
function spawnWorker(index) {
    ...
    child.stderr?.on('data', (d) => d.toString().split('\n').filter(Boolean).forEach(l => console.error(`\x1b[31m[${label} ERR]\x1b[0m ${l}`)));
    child.on('close', (code) => {
...
//# sourceMappingURL=cloud_swarm.js.map