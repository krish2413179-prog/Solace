...
async function apiRequest(apiKey, method, path, body) {
    try {
...
async function createAndActivate(apiKey, def) {
    const wf = (await apiRequest(apiKey, "POST", "/v1/workflows", def));
    await apiRequest(apiKey, "POST", `/v1/workflows/${wf.id}/go-live`);
    return wf.id;
}
function rollbackWorkflow(pipelineId, deadlineUnix, solaceAddress, chainId) {
    return {
...
function autoAcceptWorkflow(pipelineId, stepIndex, disputeBlocks, solaceAddress, chainId) {
    return {
...
function failurePropagationWorkflow(childPipelineId, parentPipelineId, parentStepIndex, solaceAddress, chainId) {
    return {
...
function childSettledWorkflow(childPipelineId, parentPipelineId, parentStepIndex, solaceAddress, chainId) {
    return {
...
export async function registerPipelineWatcher(pipelineId, deadlineUnix, stepCount, solaceAddress, chainId, parentPipelineId, parentStepIndex) {
    ...
    try {
        results.rollbackWorkflowId = await createAndActivate(apiKey, rollbackWorkflow(pipelineId, deadlineUnix, solaceAddress, chainId));
        logger.info(`KeeperHub rollback watcher LIVE | ${pipelineId.slice(0, 16)}...`);
        for (let i = 0; i < stepCount; i++) {
            const id = await createAndActivate(apiKey, autoAcceptWorkflow(pipelineId, i, 48, solaceAddress, chainId));
        ...
        if (parentPipelineId && parentStepIndex !== undefined) {
            results.failurePropagationId = await createAndActivate(apiKey, failurePropagationWorkflow(pipelineId, parentPipelineId, parentStepIndex, solaceAddress, chainId));
            logger.info(`KeeperHub failure propagation LIVE | child:${pipelineId.slice(0, 12)} → parent step ${parentStepIndex}`);
            results.childSettledNotificationId = await createAndActivate(apiKey, childSettledWorkflow(pipelineId, parentPipelineId, parentStepIndex, solaceAddress, chainId));
...
export async function cancelWatchers(watchers) {
    const apiKey = process.env.KEEPERHUB_API_KEY;
    if (!apiKey)
        return;
    const ids = [
...
export async function sendSponsoredTransaction(contractAddress, functionName, functionArgs, abi, value = "0") {
    const apiKey = process.env.KEEPERHUB_API_KEY;
    if (!apiKey) {
    ...
    const result = (await apiRequest(apiKey, "POST", "/execute/contract-call", body));
