...
const server = http.createServer(async (req, res) => {
    ...
    if (parts[0] === "channel" && parts[1]) {
        ...
        if (req.method === "POST" && parts[2] === "publish") {
            ...
            const sender = parsed.sender?.trim().toLowerCase();
            const msg_type = parsed.msg_type?.trim().toUpperCase();
            if (!sender || !msg_type)
                return send(res, 400, { error: "sender and msg_type required" });
            ensure(channelId);
            prune(channelId);
            const msg = {
                sender,
...
//# sourceMappingURL=axl_broker.js.map