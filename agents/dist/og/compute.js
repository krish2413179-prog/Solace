...
async function getBroker() {
    ...
    if (!computeKey)
        throw new Error("OG_COMPUTE_PRIVATE_KEY not set in .env");
...
async function getProviderMeta(broker) {
    if (_providerMeta)
        return _providerMeta;
    if (!config.OG_PROVIDER)
        throw new Error("OG_PROVIDER not set in .env");
...
export async function callOGCompute(systemPrompt, userPrompt, maxRetries = 5) {
    ...
    while (true) {
        ...
        if (resp.status === 429 && attempt < maxRetries) {
            ...
            logger.warn(`0G rate limit (attempt ${attempt}/${maxRetries}) — retrying in ${backoff / 1000}s...`);
