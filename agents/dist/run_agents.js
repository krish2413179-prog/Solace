...
broker.stdout?.on('data', (data) => {
    ...
    lines.forEach((line) => console.log(`\x1b[36m[Broker]\x1b[0m ${line}`));
});
broker.stderr?.on('data', (data) => {
    const lines = data.toString().split('\n').filter((l) => l.trim());
    lines.forEach((line) => console.error(`\x1b[31m[Broker ERR]\x1b[0m ${line}`));
});
...
setTimeout(() => {
    for (let i = 1; i <= WORKER_COUNT; i++) {
        ...
        worker.stderr?.on('data', (data) => {
            const lines = data.toString().split('\n').filter((l) => l.trim());
            lines.forEach((line) => console.error(`\x1b[31m[Worker ${i} ERR]\x1b[0m ${line}`));
        });
...
//# sourceMappingURL=run_agents.js.map