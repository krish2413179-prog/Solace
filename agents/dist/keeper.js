...
async function watch(pipelineId) {
    ...
    if (balance === 0n)
        throw new Error("Keeper wallet has 0 ETH");
    let errors = 0;
    while (true) {
        try {
            ...
            logger.info(`Status: ${p.statusName} | ` +
            ...
            if (now > p.deadline && [1, 2, 3].includes(p.status)) {
                ...
                try {
                    const receipt = await sendTx(() => solace.rollback(pipelineId), "rollback");
                    ...
                    if (msg.includes("reverted")) {
                        ...
                        if (fresh.status === 4 || fresh.status === 5)
                            break;
                    }
                    else {
                        throw e;
