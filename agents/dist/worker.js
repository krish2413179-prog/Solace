...
async function ensureRegistered(registry, wallet) {
    ...
    try {
        await sendTx(() => registry.register(caps, { value: minStake }), "register");
    }
    catch (e) {
        if (e.message === "SPONSORSHIP_REQUIRED") {
            logger.info("Requesting sponsored registration from KeeperHub...");
            const txHash = await sendSponsoredTransaction(await registry.getAddress(), "register", [caps], JSON.parse(readFileSync(join(process.cwd(), "registry_abi.json"), "utf8")), minStake.toString());
...
async function waitForTask(axl, address, seenPipelines, startedAfter) {
    logger.info(`Listening for task on ${CHANNEL.slice(0, 16)}...`);
    while (!aborted) {
        const msgs = await axl.poll("TASK_ASSIGNMENT", 0);
        for (const msg of msgs) {
            if (msg.timestamp < startedAfter / 1000)
                continue;
            const p = msg.payload;
            if (!p?.pipeline_id)
                continue;
            const taskKey = `${p.pipeline_id}:${p.step_index}`;
            if (seenPipelines.has(taskKey))
                continue;
            const targetAgent = p.target_agent;
            const isTargeted = targetAgent
                ? targetAgent.toLowerCase() === address.toLowerCase()
                : p.agents?.map((a) => a.toLowerCase()).includes(address.toLowerCase());
            if (isTargeted) {
...
async function waitForStepRunnable(solace, pipelineId, stepIndex) {
    logger.info(`Waiting for step ${stepIndex} to become Runnable...`);
    while (!aborted) {
        const [, , , , status] = await solace.getStep(pipelineId, stepIndex);
        const s = Number(status);
        if (s === STEP_STATUS.Runnable) {
            logger.info(`Step ${stepIndex} is Runnable`);
            return;
        }
        if (s === STEP_STATUS.Failed)
            throw new Error(`Step ${stepIndex} was marked Failed`);
        if (s === STEP_STATUS.Accepted)
            throw new Error(`Step ${stepIndex} already accepted`);
        const pipeStatus = Number(await solace.getPipelineStatus(pipelineId));
        if (pipeStatus === PIPE_STATUS.RolledBack)
            throw new Error("Pipeline rolled back");
        if (pipeStatus === PIPE_STATUS.Settled)
            throw new Error("Pipeline already settled");
        await sleep(STEP_POLL_INTERVAL);
    }
    throw new Error("Aborted while waiting for step");
}
function listenForAbort(axl) {
    (async () => {
        while (!aborted) {
...
function shouldDelegate(step, payoutEth, availableAgents) {
    const complexity = step.complexity ?? 50;
    if (complexity < DELEGATE_COMPLEXITY_THRESHOLD)
        return false;
    ...
    if (perAgentPayout < 0.001)
        return false;
    return true;
}
async function runAsSubOrchestrator(solace, registry, axl, wallet, parentId, stepIndex, step, payoutEth, deadline, minScore, pipelineType) {
    logger.info(`Acting as sub-orchestrator for step ${stepIndex}`);
    const allAvailable = await registry.getAvailableAgents();
    const cap = step.job;
    let candidates = [];
    try {
        candidates = await registry.getAgentsByCapability(cap);
    }
    catch {
        candidates = allAvailable;
    }
    candidates = candidates.filter((a) => a.toLowerCase() !== wallet.address.toLowerCase());
    const scored = [];
    for (const addr of candidates) {
        const score = await registry.getScore(addr);
    ...
    if (maxSubAgents === 0)
        throw new Error("No eligible sub-agents found");
    ...
    await sendTx(() => solace.createPipeline(childId, childDeadline, pipelineType, minScore, parentId, stepIndex, subAgents.map((a) => ethers.getAddress(a)), payoutsWei, deps, { value: totalCost }), "createChildPipeline");
    await sendTx(() => solace.linkChildPipeline(parentId, stepIndex, childId), "linkChildPipeline");
    const childSubTask = {
        pipeline_id: childId,
        parent_id: parentId,
    ...
    const registered = new Set(subAgents.map((a) => a.toLowerCase()));
    const commitMap = new Map();
    for (const msg of commits) {
        const w = msg.payload.wallet.toLowerCase();
        if (registered.has(w))
            commitMap.set(w, msg.payload.commit_hash);
    }
    if (commitMap.size < maxSubAgents) {
        throw new Error(`Missing commits: got ${commitMap.size}/${maxSubAgents}`);
    }
    const orderedHashes = subAgents.map((a) => ethers.zeroPadValue(commitMap.get(a.toLowerCase()), 32));
    await sendTx(() => solace.lockCommitments(childId, orderedHashes), "lockCommitments(child)");
    ...
    await registerPipelineWatcher(childId, childDeadline, maxSubAgents, config.SOLACE_ADDRESS, Number((await solace.runner?.provider?.getNetwork())?.chainId ?? 16600), parentId, stepIndex);
    while (!aborted) {
        ...
        if (status === PIPE_STATUS.Settled) {
            ...
            await sendTx(() => solace.notifyChildSettled(parentId, stepIndex, childId), "notifyChildSettled");
            return;
        }
...
async function sendCommit(axl, address, output) {
...
async function waitForPipelineActive(axl, solace, startedAt) {
    ...
    const pipelineId = msgs[0].payload.pipeline_id;
    const status = Number(await solace.getPipelineStatus(pipelineId));
    if (status !== PIPE_STATUS.Active)
        throw new Error(`Expected Active(2), got ${status}`);
    logger.info(`Pipeline active | ID: ${pipelineId.slice(0, 16)}...`);
    return pipelineId;
}
async function runOnce(provider, wallet, solace, registry, axl, seenPipelines, startedAfter) {
    ...
    if (!pipelineId)
        throw new Error("No pipeline_id in task payload");
    const allAvailable = (await registry.getAvailableAgents()).filter((a) => a.toLowerCase() !== wallet.address.toLowerCase());
    let outputHash;
    if (shouldDelegate(step, payoutEth, allAvailable.length)) {
        ...
        if (aborted)
            throw new Error("Aborted after step became runnable");
        await runAsSubOrchestrator(solace, registry, axl, wallet, pipelineId, stepIndex, step, payoutEth, deadline, minScore, pipelineType);
    }
    else {
        logger.info(`Executing step ${stepIndex} solo | job: ${step.job}`);
        const params = Object.fromEntries(Object.entries(step).filter(([k]) => !["job", "input", "agent_index", "complexity", "dependsOn", "maxSubAgents"].includes(k)));
    ...
    if (aborted)
        throw new Error("Aborted before submitWork");
    logger.info(`Submitting work on-chain | step: ${stepIndex} | hash: ${outputHash.slice(0, 16)}...`);
    try {
        const receipt = await sendTx(() => solace.submitWork(pipelineId, stepIndex, outputHash), "submitWork");
        logger.info(`Work submitted | TX: ${receipt.hash}`);
    }
    catch (e) {
        if (e.message === "SPONSORSHIP_REQUIRED") {
            logger.info("Requesting sponsored submission from KeeperHub...");
            const txHash = await sendSponsoredTransaction(await solace.getAddress(), "submitWork", [pipelineId, stepIndex, outputHash], JSON.parse(readFileSync(join(process.cwd(), "abi.json"), "utf8")));
    ...
    while (!aborted) {
        ...
        if (pipeStatus === PIPE_STATUS.Settled) {
            logger.info("SETTLED — payout received");
            break;
        }
        if (pipeStatus === PIPE_STATUS.RolledBack) {
            logger.warn("ROLLED BACK — no payout");
            break;
        }
        if (sStatus === STEP_STATUS.Accepted) {
            logger.info("Step accepted — awaiting full settlement");
        }
        if (sStatus === STEP_STATUS.Failed) {
